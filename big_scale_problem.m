...
m = 256; n = 128;
A = randn(m,n);
b = randn(m,1);  %x_answer = [3; -1]   (norm(A*[3;-1]-b))^2 = 225


% 函数参数含义：BB_method(start, eps, alpha, alpham, alphaM, M, c1, beta,A,b)
[K, x_min, f_min, X, F, dF_norm] = BB_method(ones(n,1), 1e-6, 1, 0.001, 20000, 3, 0.2, 0.3,A,b,m,n);
disp(['利用BB方法: 迭代次数 = ', num2str(K), ', 最优值点 x1 = ', num2str(x_min(1)),', x2 = ', num2str(x_min(2)), ', 最小值 = ', num2str(f_min)]);

[K1, x_min1, f_min1, X1, F1, dF_norm1] = DG_method(ones(n,1), 1e-6,A,b,m,n);
...
% Barzilai-Borwein 方法
function [K, x_min, f_min, X, F, dF_norm] = BB_method(start, eps, alpha, alpham, alphaM, M, c1, beta,A,b,m,n)
    % 如果没有提供参数，则使用默认值
...
% 最速下降法
function [K, x_min, f_min, X, F, dF_norm] = DG_method(start, eps,A,b,m,n)
    % 初始化记录数据
    X = zeros(n, 1000);
    F = zeros(1, 1000);
    dF = zeros(n, 1000);
